...
int bitXor(int x, int y) 
  // applying Demorgan's Law to (~x&y) || (x & ~y)
  return ~(~(~x & y) & ~(x & ~y));
}
/* 
...
int bitOr(int x, int y) {
    // applying Demorgan's Law to (x | y)
    return ~(~x & ~y);
}
/*
 ...
 */
int isZero(int x) {
  // ! operator will check whether is there any 0 in the bit form of 
  // the number 
    return !x;
...
int conditional(int x, int y, int z) {
  // if x is anything but 0, subtract 1 from
  int mask = (!x + ~0x00);
 ...
 */
int bang(int x) {
  // logical negative value of x
  // 2's compliment of x
  // if (x != 0) the most significant bit of either x or -x is 1
  return (((~x + 1) | x) >> 31) + 1; 
}
...
{
    // creating different masks for the program
    int mask1 = (0xFF << 8) | 0xFF; // mask1 = 0x0000ffff
    int mask2 = mask1 ^ (mask1 << 8); //mask2 = 0x00ff00ff
    int mask3 = mask2 ^ (mask2 << 4); //mask3 = 
    int mask4 = mask3 ^ ((mask3) << 2); //mask4
    int mask5 = mask4 ^ (mask4 << 1); // mask5
    x = x << 16 | ((x >> 16) & mask1);
    x = (x & mask2) << 8 | ((x >> 8) & mask2);
    x = (x & mask3) << 4 | ((x >> 4) & mask3);
    x = (x & mask4) << 2 | ((x >> 2) & mask4);
    x = (x & mask5) << 1 | ((x >> 1) & mask5);
    return x;
